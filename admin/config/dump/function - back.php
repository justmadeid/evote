...
function tampilcus($tampilcus){

	global $conn;
	$result = mysqli_query($conn, $tampilcus);
	$rows = [];
	while ($row = mysqli_fetch_assoc($result)) {
		$rows[] = $row;
	}
...
function tambah($data){
	...
	global $conn;
	$tambah_kategori = htmlspecialchars(trim(stripcslashes($data["namakat"])));
...
function tambahcus($data){
	...
	if (!empty($nama)) {
		$cek = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM customer WHERE nama='$nama' OR email='$email' OR telp='$telp'"));
		if ($cek > 0) {
			echo"";
		}else{
			$query = "INSERT INTO customer VALUES ('$kode','$nama','$email','$alamat','$telp','$keterangan','1')";
			mysqli_query($conn, $query);
			return mysqli_affected_rows($conn);
		}
	}
...
function upload(){
	...
	if ( !in_array($ekstensiGambar, $ekstensiGambarValid)){
		echo "<script language='javascript'>swal('Sorry', 'Infalid Exstention', 'error');</script>";
		return false;
	}

	if ($ukuranFile > 1000000) {
		echo "<script language='javascript'>swal('Sorry', 'OverSize', 'error');</script>";
		return false;
	}
...
function register($data){
	...
	mysqli_query($conn, "INSERT INTO admin VALUES('','$nama','$telp','$email','$username','$password','','1')" );
	return mysqli_affected_rows($conn);
